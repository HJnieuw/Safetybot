...
class ZoneApp:
    ...
    def __init__(self, root):
        ...
        # Mapping of activities to fixed PPE equipment and risk factors
        self.ppe_mapping = {
            'heights': ('fall protection equipment', 0.7),
            'moving vehicles': ('helmet, high visibility jacket', 0.6),
            'electricity': ('helmet, goggles, gloves, face mask, body protection', 0.5),
            'handheld vibrating tools': ('anti-vibration gloves', 0.4),
            'contaminants exposure': ('respirators', 0.3),
            'loud machinery': ('noise cancelling headsets / earplugs', 0.2),
            'chemicals exposure': ('gloves, goggles, respirator', 0.1)
        }
        ...
        self.coord_label.grid(row=0, column=0, sticky='w')
        ...
        # Zone name entry
        tk.Label(self.frame_right, text="Zone Name:", font=('TkDefaultFont', 9, 'bold')).grid(row=2, column=0, sticky='w')
        ...
        # Activity combobox
        tk.Label(self.frame_right, text="Activity:", font=('TkDefaultFont', 9, 'bold')).grid(row=4, column=0, sticky='w')
        tk.Label(self.frame_right, text="working with  ").grid(row=5, column=0, pady=5, padx=5, sticky='w')
        self.activity_combobox = ttk.Combobox(self.frame_right, width=22)
        self.activity_combobox['values'] = list(self.ppe_mapping.keys())
        self.activity_combobox.grid(row=5, column=0, pady=5, padx=5, sticky="e")
        self.activity_combobox.bind('<<ComboboxSelected>>', self.display_ppe_for_activity)

        # PPE display
        tk.Label(self.frame_right, text="Required PPE:", font=('TkDefaultFont', 9, 'bold')).grid(row=6, column=0, sticky='w')
        self.ppe_display_label = tk.Label(self.frame_right, text="", anchor='w') 
        self.ppe_display_label.grid(row=7, column=0, pady=5, padx=5, sticky='w')

        # Risk factor display
        tk.Label(self.frame_right, text="Risk Factor:", font=('TkDefaultFont', 9, 'bold')).grid(row=8, column=0, sticky='w')
        self.risk_factor_label = tk.Label(self.frame_right, text="", anchor='w')  
        self.risk_factor_label.grid(row=9, column=0, pady=5, padx=5, sticky='w')
    ...
    # Display PPE and Risk Factor based on selected activity
    def display_ppe_for_activity(self, event):
        activity = self.activity_combobox.get()
        required_ppe, risk_factor = self.ppe_mapping.get(activity, ("No PPE defined", 0))
        self.ppe_display_label.config(text=required_ppe)
        self.risk_factor_label.config(text=f"{risk_factor:.1f}")

    # Submit details (per zone)
    def submit_details(self):
        zone_name = self.zone_name_entry.get().strip()
        zone_activity = self.activity_combobox.get().strip()
        required_PPE, risk_factor = self.ppe_mapping.get(zone_activity, ("No PPE defined", 0))
    ...
    def load_selected_zone(self, event):
        ...
        if selected_zone in self.zone_id:
            ...
            self.ppe_display_label.config(text=zone_data.get("required_PPE", ""))
            self.risk_factor_label.config(text=f"{zone_data.get('risk_factor', 0):.1f}")
    ...
    def clear_fields(self):
        ...
        self.risk_factor_label.config(text="")
...
class ZoneOverview:
    ...
    def __init__(self, root):
        ...
        self.risk_factor_label = tk.Label(self.info_frame, text="Risk Factor: ")
    ...
    def show_zone_info(self, zone_data, zone_name):
        ...
        self.risk_factor_label.config(text=f"Risk Factor: {zone_data.get('risk_factor', 0):.1f}")
