...
# Load the image
image_path = r"C:\Users\Barte van der Zijden\Downloads\Foto1-1.png"  # Update the path as necessary
...
# Use adaptive thresholding with a finer block size and constant value to detect walls better
thresholded = cv2.adaptiveThreshold(img, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 11, 2)

# Optionally, use Canny edge detection to better identify edges (walls)
edges = cv2.Canny(img, 50, 150)

# You can use either 'thresholded' or 'edges' below, depending on which works better
...
# Display the floorplan with binary colormap (better visualization)
plt.imshow(floorplan, cmap='binary', vmin=0, vmax=1)
...
fig, ax = plt.subplots()
ax.imshow(floorplan, cmap='binary', vmin=0, vmax=1)  # Use binary colormap for better visualization
