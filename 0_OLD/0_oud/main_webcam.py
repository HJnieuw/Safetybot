...
while True:
    ...
    # Add hazard information to the zone_ID
    if zone_name in zone_data:
        if "amount of hazards" in zone_data[zone_name]:
            zone_data[zone_name]["amount of hazards"] += hazard_count  # Add to existing hazard count
        else:
            zone_data[zone_name]["amount of hazards"] = hazard_count  # Initialize if not present
        if "hazard type" in zone_data[zone_name]:
            zone_data[zone_name]["hazard type"].extend(hazards)  # Append new hazards to the list
        else:
