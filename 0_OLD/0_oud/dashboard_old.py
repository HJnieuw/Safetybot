...
class ConstructionHazardVisualizer:
    def __init__(self):
        ...
        self.fig, self.ax1 = plt.subplots(1, 1, figsize=(12, 6))  # Only one subplot for the heatmap
        self.ax_text = None  # Placeholder for the text Axes
    ...
    # Function to display a simple text message under the image
    def display_message(self, zone_name, helmet_hazard_count):
        # Clear previous text message if it exists
        if self.ax_text:
        ...
        plt.draw()  # Redraw the figure to update the changes
        ...
        if event.inaxes == self.ax1:
            ...
            for zone_name, info in self.zone_data.items():
                ...
                if np.hypot(x_click - x_zone, y_click - y_zone) < 50:  # Threshold for detecting the click
                    # Directly use 'amount_of_hazards' for helmet hazard count
